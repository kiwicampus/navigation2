...
ResultStatus Spin::onRun(const std::shared_ptr<const SpinActionGoal> command)
{
  ...
  // Kiwi customization (#24): check the current pose is collision-free before
  // starting the spin, to avoid spinning on a deadly cell. Reconciled to use the
  // same collision-checker API as Spin::isCollisionFree below.
