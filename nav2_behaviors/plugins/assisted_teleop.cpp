...
AssistedTeleop::AssistedTeleop()
    : TimedBehavior<AssistedTeleopAction>(), feedback_(std::make_shared<AssistedTeleopAction::Feedback>())
{
}
...
void AssistedTeleop::onConfigure()
{
    ...
    // set up parameters
    nav2_util::declare_parameter_if_not_declared(node, "projection_time", rclcpp::ParameterValue(1.0));

    nav2_util::declare_parameter_if_not_declared(node, "simulation_time_step", rclcpp::ParameterValue(0.1));

    nav2_util::declare_parameter_if_not_declared(node, "cmd_vel_teleop",
                                                 rclcpp::ParameterValue(std::string("cmd_vel_teleop")));
...
Status AssistedTeleop::onCycleUpdate()
{
    ...
    for (double time = simulation_time_step_; time < projection_time_; time += simulation_time_step_)
    {
        ...
        if (!collision_checker_->isCollisionFree(projected_pose))
        {
            ...
            {
                RCLCPP_DEBUG_STREAM_THROTTLE(logger_, steady_clock_, 1000,
            ...
            else
            {
                RCLCPP_DEBUG_STREAM_THROTTLE(logger_, steady_clock_, 1000,
                                             behavior_name_.c_str()
                                                 << " collision approaching in " << time << " seconds");
...
geometry_msgs::msg::Pose2D AssistedTeleop::projectPose(const geometry_msgs::msg::Pose2D& pose,
                                                       const geometry_msgs::msg::Twist& twist, double projection_time)
{
    geometry_msgs::msg::Pose2D projected_pose = pose;

    projected_pose.x += projection_time * (twist.linear.x * cos(pose.theta) + twist.linear.y * sin(pose.theta));

    projected_pose.y += projection_time * (twist.linear.x * sin(pose.theta) - twist.linear.y * cos(pose.theta));
...
void AssistedTeleop::teleopVelocityCallback(const geometry_msgs::msg::Twist::SharedPtr msg) { teleop_twist_ = *msg; }

void AssistedTeleop::preemptTeleopCallback(const std_msgs::msg::Empty::SharedPtr) { preempt_teleop_ = true; }
