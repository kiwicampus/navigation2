...
TEST(VelocitySmootherTest, openLoopTestTimer)
{
    ...
    auto subscription = smoother->create_subscription<geometry_msgs::msg::Twist>(
        "cmd_vel_smoothed", 1, [&](geometry_msgs::msg::Twist::SharedPtr msg) { linear_vels.push_back(msg->linear.x); });
...
TEST(VelocitySmootherTest, testDynamicParameter)
{
    ...
    auto results = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("smoothing_frequency", 100.0), rclcpp::Parameter("feedback", std::string("CLOSED_LOOP")),
         ...
         rclcpp::Parameter("max_decel", min_accel), rclcpp::Parameter("odom_topic", std::string("TEST")),
         rclcpp::Parameter("odom_duration", 2.0), rclcpp::Parameter("velocity_timeout", 4.0),
