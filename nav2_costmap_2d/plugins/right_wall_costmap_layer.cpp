                                             ...
                                             double* max_y)
{
  ...
  *max_y = std::max(tmp_max_y, *max_y);
  ...
  double distance_to_goal = distance(robot_x, robot_y, goal_x_, goal_y_);
  ...
  goal_x_ = global_path_->poses[path_index_+1].pose.position.x;
  goal_y_ = global_path_->poses[path_index_+1].pose.position.y;
  RCLCPP_WARN(logger_, "RightWallCostmapLayer::updateBounds() resolution: %f", map_resolution_);
  ...
  // print static map origin
  // RCLCPP_WARN(logger_, "RightWallCostmapLayer::updateBounds() static_map origin: %f, %f", static_map_->info.origin.position.x, static_map_->info.origin.position.y);

  // RCLCPP_WARN(logger_, "RightWallCostmapLayer::updateBounds() robot_x: %f, robot_y: %f", prev_goal_x_, prev_goal_y_);
  // RCLCPP_WARN(logger_, "RightWallCostmapLayer::updateBounds() goal_x: %f, goal_y: %f", goal_x_, goal_y_);
  // RCLCPP_WARN(logger_, "RightWallCostmapLayer::updateBounds() min_i: %d, min_j: %d, max_i: %d, max_j: %d", min_i, min_j, max_i, max_j);
