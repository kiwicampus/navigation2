...
// New function to build and publish the optimal path
void TrajectoryVisualizer::buildAndPublishOptimalPath(
  const xt::xtensor<float, 2> & trajectory,
  const builtin_interfaces::msg::Time & cmd_stamp)
{
  if (optimal_path_pub_->get_subscription_count() > 0) {
    // Build the optimal path message
    ...
    for (size_t i = 0; i < trajectory.shape()[0]; ++i) {
      geometry_msgs::msg::PoseStamped pose_stamped;
      pose_stamped.header.frame_id = frame_id_;
      pose_stamped.header.stamp = cmd_stamp;

      // Set the position using trajectory data
      pose_stamped.pose.position.x = trajectory(i, 0);
      pose_stamped.pose.position.y = trajectory(i, 1);
      pose_stamped.pose.position.z = 0.0;

      // Set the orientation using the yaw from trajectory data
      tf2::Quaternion quaternion_tf2;
      quaternion_tf2.setRPY(0.0, 0.0, trajectory(i, 2));
      pose_stamped.pose.orientation = tf2::toMsg(quaternion_tf2);
