...
void PathAlignCritic::score(CriticData& data)
{
    ...
    const auto T_x = xt::view(data.trajectories.x, xt::all(), xt::range(0, _, trajectory_point_step_));
    const auto T_y = xt::view(data.trajectories.y, xt::all(), xt::range(0, _, trajectory_point_step_));
    const auto T_yaw = xt::view(data.trajectories.yaws, xt::all(), xt::range(0, _, trajectory_point_step_));
